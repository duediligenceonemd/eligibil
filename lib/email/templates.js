...
var T = {
  ...
  passwordReset: {
    subject: { ro: 'Resetează parola contului tău eligibil.org', en: 'Reset your eligibil.org password', ru: 'Сбросьте пароль eligibil.org', ua: 'Скиньте пароль eligibil.org' },
    title:   { ro: 'Resetează parola', en: 'Reset your password', ru: 'Сбросьте пароль', ua: 'Скиньте пароль' },
    body:    {
      ro: 'Am primit o solicitare de resetare a parolei pentru contul tău eligibil.org. Dacă ai făcut tu această solicitare, folosește butonul de mai jos pentru a seta o parolă nouă.',
      en: 'We received a request to reset your eligibil.org password. If this was you, use the button below to set a new password.',
      ru: 'Мы получили запрос на сброс пароля eligibil.org. Если это были вы, используйте кнопку ниже, чтобы задать новый пароль.',
      ua: 'Ми отримали запит на скидання пароля eligibil.org. Якщо це були ви, скористайтеся кнопкою нижче, щоб встановити новий пароль.',
    },
    expiry: {
      ro: 'Linkul expiră în {minutes} minute. Dacă nu ai cerut resetarea parolei, poți ignora acest email.',
      en: 'This link expires in {minutes} minutes. If you did not request a password reset, you can ignore this email.',
      ru: 'Ссылка действительна {minutes} минут. Если вы не запрашивали сброс пароля, просто проигнорируйте это письмо.',
      ua: 'Посилання діє {minutes} хвилин. Якщо ви не запитували скидання пароля, просто проігноруйте цей лист.',
    },
    cta: { ro: 'Setează parola nouă', en: 'Set new password', ru: 'Задать новый пароль', ua: 'Задати новий пароль' },
  },
  footer: {
...
function passwordReset(opts) {
  var lang = opts.language || 'ro';
  var t = T.passwordReset;
  var expiryText = txt(t.expiry, lang).replace('{minutes}', String(opts.expiresInMinutes || 30));
  var content =
    heading(txt(t.title, lang)) +
    paragraph(txt(t.body, lang)) +
    button(opts.resetUrl || '#', txt(t.cta, lang)) +
    smallText(expiryText);
  return layout(lang, null, content);
}
