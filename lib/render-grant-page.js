...
function renderGrantPage(grant, lang) {
  ...
  // ── Rich JSON-LD: FinancialProduct + BreadcrumbList ──────────────────────
  const jsonLd = {
    '@context': 'https://schema.org',
    '@type': 'FinancialProduct',
    name: title,
    description: description || (grant.descriere || '').slice(0, 300),
    url: canonical,
    provider: funderName ? {
      '@type': 'Organization',
      name: funderName,
      url: grant.source_url || undefined,
    } : undefined,
    isAccessibleForFree: true,
  };
  // Amount range
  const currency = grant.valuta || 'EUR';
  if (grant.suma_maxima || grant.suma_max) {
    jsonLd.amount = {
      '@type': 'MonetaryAmount',
      currency,
      minValue: grant.suma_minima || grant.suma_min || undefined,
      maxValue: grant.suma_maxima || grant.suma_max || undefined,
    };
  }
  // Deadline
  if (grant.deadline) jsonLd.applicationDeadline = grant.deadline;
  // Eligible countries
  if (grant.tara) {
    jsonLd.eligibleRegion = String(grant.tara).split(/[,;]/).map(c => ({
      '@type': 'Country',
      name: c.trim(),
    }));
  }
  // Sector
  if (grant.sector) jsonLd.category = grant.sector;
  ...
  // x-default points to English (lingua franca for non-RO/RU/UA users)
  const xDefaultUrl = enUrl || roUrl;
  const seoTags = [
    '<link rel="canonical" href="' + escapeHtml(canonical) + '" />',
    roUrl ? '<link rel="alternate" hreflang="ro" href="' + escapeHtml(roUrl) + '" />' : '',
    roUrl ? '<link rel="alternate" hreflang="ro-MD" href="' + escapeHtml(roUrl) + '" />' : '',
    roUrl ? '<link rel="alternate" hreflang="ro-RO" href="' + escapeHtml(roUrl) + '" />' : '',
    enUrl ? '<link rel="alternate" hreflang="en" href="' + escapeHtml(enUrl) + '" />' : '',
    xDefaultUrl ? '<link rel="alternate" hreflang="x-default" href="' + escapeHtml(xDefaultUrl) + '" />' : '',
  ...
  // Inject chat widget script before </body> on SSR grant pages
  if (!html.includes('chat-widget.js')) {
    html = html.replace('</body>', '<script src="/chat-widget.js?v=20260522b" defer></script>\n</body>');
  }
...
function renderEnhancedSections(g, lang) {
  ...
  // Related grants from same funder (Funder Intelligence)
  const related = Array.isArray(g.related_grants) ? g.related_grants : [];

  return `
<style>
  .gp-enh { max-width: 1200px; margin: 0 auto; padding: 48px 24px; font-family: Inter, system-ui, sans-serif; }
  .gp-enh__why { display: inline-flex; align-items: center; gap: 6px; padding: 8px 14px; background: #1f3a5f; color: #fff; text-decoration: none; font-family: 'JetBrains Mono', monospace; font-size: 11px; text-transform: uppercase; letter-spacing: .08em; border: none; cursor: pointer; border-radius: 3px; margin-bottom: 24px; }
  .gp-enh__why:hover { background: #16294a; }
  .gp-enh-drawer { display: none; position: fixed; inset: 0; background: rgba(0,0,0,.5); z-index: 100; align-items: center; justify-content: center; padding: 24px; }
  .gp-enh-drawer.is-open { display: flex; }
  .gp-enh-drawer__panel { background: #fff; max-width: 720px; width: 100%; max-height: 85vh; overflow-y: auto; border-radius: 4px; padding: 32px; }
  .gp-enh-drawer__close { float: right; background: none; border: none; font-size: 24px; cursor: pointer; color: #555; }
  .gp-enh-drawer h2 { font-family: 'Space Grotesk', sans-serif; font-size: 24px; margin: 0 0 6px; }
  .gp-enh-drawer h3 { font-family: 'Space Grotesk', sans-serif; font-size: 16px; margin: 18px 0 6px; color: #1f3a5f; }
  .gp-enh-drawer dl { margin: 0; }
  .gp-enh-drawer dt { font-family: 'JetBrains Mono', monospace; font-size: 11px; color: #555; text-transform: uppercase; letter-spacing: .08em; margin-top: 8px; }
  .gp-enh-drawer dd { margin: 4px 0 0; font-size: 14px; color: #0e1620; }
  .gp-enh-drawer__sig { display: inline-block; padding: 2px 8px; background: #f7f5f0; border: 1px solid #d4cfc4; font-family: 'JetBrains Mono', monospace; font-size: 11px; margin-right: 4px; color: #555; }
  .gp-enh__funder-grid { display: grid; grid-template-columns: repeat(2, 1fr); gap: 14px; }
  @media (max-width: 700px) { .gp-enh__funder-grid { grid-template-columns: 1fr; } }
  .gp-enh__funder-card { padding: 18px; background: #f7f5f0; border: 1px solid #d4cfc4; text-decoration: none; color: inherit; transition: border-color .15s; }
  .gp-enh__funder-card:hover { border-color: #1f3a5f; }
  .gp-enh__funder-card-name { font-weight: 600; font-size: 15px; color: #0e1620; }
  .gp-enh__funder-card-meta { font-family: 'JetBrains Mono', monospace; font-size: 11px; color: #555; margin-top: 4px; }
  .gp-enh__funder-card-sub { font-size: 13px; color: #555; margin-top: 8px; line-height: 1.5; }
