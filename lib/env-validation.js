...
function rejectUnsafeSupabaseServerKey(name) {
  const value = process.env[name];
  if (!value) return null;

  if (/^sb_publishable_|^sb_anon_/i.test(value)) {
    throw new Error(`${name} must use a Supabase service role key on the server, not a publishable/anon key`);
  }
...
function validateEnv() {
  ...
  if (isProduction) {
    ...
    rejectUnsafeSupabaseServerKey('SUPABASE_SERVICE_KEY');
  }
