...
const DEFAULT_CHAT_MODEL = process.env.AWS_CHAT_MODEL || 'us.anthropic.claude-sonnet-4-5-20250929-v1:0';
 ...
 * @param {string}        opts.userMessage  - single user message text (used when no opts.messages)
 * @param {Array}         opts.messages     - full multi-turn messages array [{role, content}]
 *                                           When provided, userMessage is ignored.
...
async function bedrockChat(opts) {
  ...
  // Build messages — prefer opts.messages (multi-turn), fall back to single userMessage
