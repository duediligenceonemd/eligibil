...
function faqEntries(lang, sectorLabel, countryLabel) {
  if (lang === 'en') return [
    ...
    {
      q: `How does eligibil.eu's match score work?`,
  ...
  return [
    ...
    {
      q: `Cum funcționează scorul de potrivire eligibil.eu?`,
...
function renderSeoListing({ lang, sectorKey, sectorLabel, countryKey, countryLabel, grants }) {
  ...
  const seoTitle = (isEn
    ? sectorLabel + ' grants for startups in ' + countryLabel
    : 'Granturi ' + sectorLabel + ' pentru startupuri din ' + countryLabel
  ) + ' | eligibil.eu';

  const seoDesc = isEn
    ? `${n} active funding programs for ${sectorLabel} startups in ${countryLabel}. Verified weekly. Match scores when signed in.`
    : `${n} programe active de finanțare pentru startupurile ${sectorLabel} din ${countryLabel}. Verificat săptămânal. Scor de potrivire la autentificare.`;
  ...
  return (
    ...
    '<header class="sb__top">\n' +
      '<a class="sb__brand" href="/">eligibil<span>.eu</span></a>\n' +
      '<nav class="sb__nav">\n' +
    ...
    '<section class="sb__hero">\n' +
      '<div class="sl__crumbs"><a href="/">eligibil.eu</a> ' + escapeHtml(t.breadcrumb_arrow) + ' <a href="/search">' + escapeHtml(t.crumbs_root) + '</a> ' + escapeHtml(t.breadcrumb_arrow) + ' <span>' + escapeHtml(sectorLabel) + ' / ' + escapeHtml(countryLabel) + '</span></div>\n' +
